...
export const useMusicStore = create<
  MusicState & MusicAction & AudioAction & AudioState,
...
>(
  persist<FullState, [], [], MyPersist>(
    (set, get) => ({
      ...
      onTimeUpdate: () => {
        GlobalAudio.ontimeupdate = () => {
          ...
          if (lyric != undefined) {
            ...
            if (
              rencentLyricIndex >= 0 &&
              rencentLyricIndex != get().currentLyricIndex
            ) {
              set(() => ({
      ...
      setCurrentPlaylistId: (id: number) =>
        set(() => ({
      ...
      generateNextId: (next: boolean) => {
        return new Promise<void>((resolve) => {
          ...
          const currentIndex = currentMusicIds.indexOf(
            get().currentMusicData?.id || 0
          );
          let nextIndex = 0;
          switch (get().playMode) {
            case PlayMode.REPEAT:
