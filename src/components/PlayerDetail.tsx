...
import {FontDownload, FontDownloadOutlined, KeyboardArrowDown} from "@mui/icons-material";
import {MusicMainController, MusicSlider, RightController} from "./BottomPlayer.tsx";
    ...
    return <>
        ...
        >
            <Box sx={{position: "absolute", top: 0, right: 0, zIndex: 1}} p={2}>
                <RoundedIconButton icon={<KeyboardArrowDown/>} onClick={() => {
                    setExpand(false)
                }}/>
            </Box>
                ...
                >
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{mr: 2}} flex={1}>
                            ...
                            <Typography variant="subtitle1" sx={{fontSize: 12}} color="text.secondary" noWrap>
                                {currentMusicData?.ar[0]?.name || '未知艺术家'} - {currentMusicData?.al.name || '未知专辑'}
                            </Typography>
                        </Box>
                        <Box flex={1}>
